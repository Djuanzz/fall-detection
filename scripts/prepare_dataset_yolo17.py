...
def main():
    ...
    if args.split_method == "cross_subject":
        ...
        # Subjek di luar 1-20 → fallback ke random split
        unknown = [(fp, lbl, stem) for fp, lbl, stem in all_samples
                   if get_subject_id(stem) not in XSUB60_TRAIN | XSUB60_TEST]
        if unknown:
            ul = [s[1] for s in unknown]
            u_tr, u_va = train_test_split(
                unknown, test_size=args.val_split,
                stratify=ul, random_state=args.seed)
            tr_all += u_tr
            va_all += u_va
            print(f"  [INFO] {len(unknown)} sampel subjek >20 di-split random")
    else:
